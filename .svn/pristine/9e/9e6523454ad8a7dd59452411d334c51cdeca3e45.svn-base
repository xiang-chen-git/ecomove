estimateCVM.onestep <- function(Z, T, CI = FALSE, parameters = c("tau", "nu"), spline = FALSE, like=FALSE)
{   
  V <- diff(Z)/diff(T)
  ...
  if(!"nu" %in% parameters)
  {
    ...
    logtau.fit <- optimize(V.LogLikelihood.Tau, c(0, diff(range(T))/2))
    logtau.hat <- logtau.fit$min
    logtau.sd <- 1/sqrt(hessian(V.LogLikelihood.Tau, logtau.hat, Z=Z, T=T))
